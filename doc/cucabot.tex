...
%(-90:-90-\BallBeta:\BallRadius/5.0) node[pos=0,auto=left]{$\beta$};
	...
	\draw [dotted] (Bx) circle (\BallRadius/3);
%	\node[right] at (Bx) +(-90:\BallRadius/5)($(O) 
%		+(-\RobotThetaSonar/4:\SonarS/5)$) {$\theta_2$};
	\draw [dotted] (O) circle (\BallRadius/5);
	\node[above] at ($(Bx) +(+90-\BallBeta/2:\BallRadius/3)$) {$\beta$};
	\end{scope}
	\begin{scope}
	\path[clip] (By) -- (O) -- (Cy) -- cycle;
	\draw [dotted] (By) circle (\BallRadius/4);
%	\node[right] at (Bx) +(-90:\BallRadius/5)($(O) 
%		+(-\RobotThetaSonar/4:\SonarS/5)$) {$\theta_2$};
	\draw [dotted] (O) circle (\BallRadius/5);
	\node[right] at ($(By) +(+45-\BallBeta/2:\BallRadius/4)$) {$\delta$};
	\end{scope}
