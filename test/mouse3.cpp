...
void handle_bit()
{
	...
	*/
	...
	if(rx_actual_bit == 0)
	{
		...
		Timer1.attachInterrupt(handle_bit);
	}
	// End of data bits ?
	else if(rx_actual_bit == 6)
	{
		...
		Timer1.stop();
		Timer1.detachInterrupt();
		Timer1.initialize(833*2);
		Timer1.attachInterrupt(handle_last_bit);
		//if(EIMSK & (1 << INT0)) err();
	}
...
void handle_interrupt()
{
	...
	*/
	...
	// Set PWM, Phase and Frequency Correct
	// and prescaler = 1
	TCCR1B = _BV(WGM13) | _BV(CS10);
	*/
	Timer1.attachInterrupt(handle_bit);
	Timer1.initialize(416+833);
}
